$(function(){
    ...
    $('.loginBtnLogin').click(function(){
        ...
        $.ajax({
            url : "http://localhost:8080/newTest/js/login.jsp",
            data : {id:id, pw:pw},
            dataType : 'jsonp',
    ...
    $('.joinBtnJoin').click(function(){
        ...
        isJoin = true; // 회원가입 시도
        $.ajax({
            url : "http://localhost:8080/newTest/js/register.jsp",
            data : {id:id, pw:pw},
            dataType : 'jsonp',
            success : function(data) {
                console.log(data);
    ...
    var loadPosts = function(){
        ...
        // 데이터를 AJAX로 불러옵니다.
        $.ajax({
            url : "http://localhost:8080/newTest/js/load.jsp",
            data : {},
            dataType : 'jsonp',
            success : function(data){
                if(data.result == "success") {
    ...
    var loadComment = function(postId){
        ...
        // 데이터를 AJAX로 불러옵니다.
        $.ajax({
            url : "http://localhost:8080/newTest/js/commentLoad.jsp",
            data : {postId : postId},
            dataType : 'jsonp',
            success : function(data){
                if(data.result == "success"){
    ...
    $(document.body).on('click', '.mainBtnDel', function(){
        if(confirm('삭제?')){
            ...
            $.ajax({
                url : "http://localhost:8080/newTest/js/del.jsp",
                data : {postId : id},
                dataType : 'jsonp',
                success : function(data){
                    if(data.result == "success") {
    ...
    $(document.body).on('click', '.commentBtnWrite', function(){
        ...
        $.ajax({
            url : "http://localhost:8080/newTest/js/commentPost.jsp",
    ...
    $(document.body).on('click', '.commentBtnDel', function(){
        if(confirm('댓글 삭제?')){
            ...
            $.ajax({
                url : "http://localhost:8080/newTest/js/commentDel.jsp",
                data:{postId:id},
                dataType:'jsonp',
                success:function(data){
                    if(data.result == "success")
    ...
    $('.writeBtnWrite').click(function(){
        ...
        if(window.confirm('글을 작성하시겠습니까?')){
            isPost = true;
            $.ajax({
                url : "http://localhost:8080/newTest/js/post.jsp",
                data : {
                    subject : subject,
                    content : content,
                    writer : currentUser
